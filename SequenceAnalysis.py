# !/usr/bin/env python3
# Name: Justin Chan (jumchan)
# Group Members: None
"""
The class, ProteinParam, is a class that is used to output many characteristics
of an amino acid sequence.

The class has plently of methods that calculate their own characteristics of the 
input amino acid sequence and returns them to the ProteinParam object created in 
the main method.

input: an input string that represents an amino acid sequence.
output: an output formatted message with characteristics of the amino acid sequence.

There are not many assumptions but, I would believe there should at least be one
valid amino acid in the sequence or most values returned will be zero.
"""
class ProteinParam :
    """
    ...
    The class contains seven attributes that were already coded for the purpose
    of this class. 4 of these class attibutes are dictionaries that contain values
    of different coefficients, weights, and pKas that are associated with one amino
    acid letter as keys. These seven class attributes are made accessable in 
    methods to be able to be used to calculate the quantitative characteristics.
    
    ProteinParam takes an input string that represents an amino acid sequence
    which excludes all characters that are not the single amino acid letter
    codes and saves the count of each amino acid in the input sequence in a
    dictionary. This was decided because this was the optimal design to having
    the most efficient class given the methods
    
    All methods in the classes return either integers and floats that are to be
    printed line by line by the main method in a formatted way to present the 
    amino acid sequence with an exception to aaComposition that returns a
    dictionary of amino acid one letter codes and counts of each amino acid.
    ...
    def charge (self, pH):
        ...
        if self.aaCount() != 0:
            for aminoAcid, countAminoAcid in self.countAA.items(): # Loops through amino acids
                if self.aa2chargePos.get(aminoAcid, 'unknown') != 'unknown': # Checks for positively charged amino acids
                    posCharge += countAminoAcid * (10**(self.aa2chargePos.get(aminoAcid)))/(10**self.aa2chargePos.get(aminoAcid) + 10**pH)
                elif self.aa2chargeNeg.get(aminoAcid,'unknown') != 'unknown': # Checks for negatively charged amino acids
...
# Group Members: None
'''
The class, NucParams, is a class that is used to count many different attributes of a 
DNA/RNA sequence such as single nucleotides, codons, and amino acids.

The class has many methods that make counts of codons, nucleotides, and amino acids
in the DNA/RNA nucleotide kept in dictionaries and returns them.

input: a string that represents a DNA/RNA nucleotide sequence (could be an empty string)
output: No strict output. Used in another class to created a formatted output message with
counting methods in class.

There are not many assumptions other than the DNA/RNA nucleotide sequence that is entered 
can have white spaces and be lower or upper case.
'''
class NucParams:
    '''
    ...
    '''
    # This rnaCodonTable dictionary was already coded into the class.
    # It is essential because it gives all codon combinations and its corresponding amino
    # acid codes.
    # More specifically, it is useful in aaCompositon() method to convert codons to amino
    # acid counts.
    rnaCodonTable = {
    # RNA codon table
    # U
    ...
    # dnaCodonTable is similar to the rnaCodonTable but, for DNA.
    # It is not used but, could be very useful.
    ...
    def addSequence (self, inSeq):
        ...
        # Loops through single nucleotide characters
        # Adds nucleotide to validNucDic dictionary if it is a valid nucleotide
        for singNuc in addedDNANoSpace:
            if singNuc in self.validNucDic:
                self.validNucDic[singNuc] += 1 # Adds to count
        
        # Makes a complete RNA Sequence by changing T's to U's
        rnaSeqConv = addedDNANoSpace.replace('T', 'U')
        
        # Loops through a string of three characters that make a codon
        # If a valid codon, adds to count in the rnaCodonCount dictionary
    ...
    def aaComposition(self):
        ...
        Creates a dictionary called aaCount which is an dictionary with all amino acids
        ...
        to a item in aaCount if it determines a codon belongs to a particular amino acid.
        ...
        '''
        aaCount = {
        # Amino Acid Count Dictionary
        'F': 0, 'L': 0, 'I': 0, 'M': 0, 'V': 0,
        'S': 0, 'P': 0, 'T': 0, 'A': 0, 'Y': 0,
        'H': 0, 'Q': 0, 'N': 0, 'K': 0, 'D': 0,
        'E': 0, 'C': 0, 'W': 0, 'R': 0, 'G': 0, '-': 0
        }
        # Loops through each codon to check if it has a count
        for codonCode in self.rnaCodonCount.keys():
            if self.rnaCodonCount.get(codonCode) != 0: #Checks if count is not 0
                aaCount[self.rnaCodonTable.get(codonCode)] += self.rnaCodonCount.get(codonCode)
                # Adds to codon count to aaCount item
...
'''
The purpose of the class, OrfFinder, is to be able to determine the positions, lengths,
and frames of open readings frames (ORFS).

The class is made up of many small methods but, the most of the program is done by 
the method, findOrfs(), which finds the positions and length of ORFS and adds them 
to the class attribute, openReadingFrames, a list that contains lists of start and
stop positions, frames, and lengths of ORFs in the sequence.

input: A string that represents a DNA sequence
output: A list of lists that contain
    - the start and stop positions of the ORF
    - the frame the ORF was found in
    - the length of the ORF

Assumptions:
    - Strictly DNA sequences can be entered
    If the input file that represents DNA has any nucleotides that are not A, C, G
    or T, the OrfFinder will produce an error. Other than that, there was not any 
    other assumptions.
'''
class OrfFinder():
    '''
    ...
    def reverseComplimentaryStrand (self, revStrand):
        '''
        Creates a reversed complimentary strand of a DNA sequence.
        
        input: A string of a DNA sequence that will be used as a template strand.
        output: A reversed complimentary DNA sequence of the input DNA sequence.
        
        Keyword Arguments:
        revStrand -- the input, template DNA sequence used to generate the complimentary
        reversed strand
        '''
        complimentNucs = {'A':'T', 'T':'A', 'G':'C', 'C':'G'}
        complimentStrand = ""
        
        # Loops through every nucleotide of a DNA sequence and adds the matching
        # base pair nucleotide to the reversed complimentary strand
        for nuc in revStrand:
            complimentStrand += complimentNucs.get(nuc)
                ...
                #Checks for stop codons
                    ...
                    if noStopYet == True and len(startPos) == 0 and minLength < endPos: 
                        if typeStrand > 0: # Checks for positive frames
                            ...
                            self.openReadingFrames.append([frame * typeStrand, len(strand) - (endPos - 1), len(strand), endPos])
                        ...
                        for pos in startPos:
                            ...
                            if minLength < (endPos - pos + 1):
                                if typeStrand > 0:# Checks for positive frames
                                    ...
                                    self.openReadingFrames.append([frame * typeStrand, len(strand) - (endPos - 1), len(strand) - (pos - 1), endPos - pos + 1])
                ...
                for pos in startPos:
                    ...
                    if minLength < (len(strand) - frame + 2 - pos):
                        if typeStrand > 0:# Checks for positive frames
                            ...
                            self.openReadingFrames.append([frame * typeStrand, 1, len(strand) - (pos - 1), len(strand) - pos + 1])
